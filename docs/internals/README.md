...
For example, a change to call-dependent remapping begins in the linker's
internal [`addDependency()`](../../src/compiler/graphLinker.cpp), which
records a destination `Expression *` in `UnaryCallExpression::depRemaps`. The
corresponding serializer in
[`UnaryCallExpression::toByteCode()`](../../src/compiler/expression.cpp)
resolves that identity to the dependent's emitted index. The focused
`linkGraph.setsCallDepRemaps` test in
[`tests/compiler/graphLinker.cpp`](../../tests/compiler/graphLinker.cpp) checks
the linker representation, while the generated bytecode must also be parsed
and exercised by an executor or E2E test. This sequence illustrates the
minimum cross-boundary evidence expected before work on that feature is
considered complete.
